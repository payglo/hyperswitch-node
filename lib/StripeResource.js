...
const {
  ...
  PaygloConnectionError,
  StripeAuthenticationError,
  PaygloAuthenticationError,
...
StripeResource.prototype = {
        ...
        .then(
          (jsonResponse) => {
            if (jsonResponse.error) {
              ...
              if (statusCode === 401) {
                err = new PaygloAuthenticationError(jsonResponse.error);
              } else if (statusCode === 403) {
                err = new StripePermissionError(jsonResponse.error);
              } else if (statusCode === 429) {
                err = new StripeRateLimitError(jsonResponse.error);
              } else {
                err = PaygloError.generate(jsonResponse.error);
              }
          ...
          (e) => {
            throw new PaygloAPIError({
              message: 'Invalid JSON received from the Payglo API',
              exception: e,
  ...
  _generateConnectionErrorMessage(requestRetries) {
    return `An error occurred with our connection to Payglo.${
  ...
  _makeHeaders(
    ...
    const defaultHeaders = {
      ...
      'api-key': auth ? `${auth}` : this._stripe.getApiField('auth'),
